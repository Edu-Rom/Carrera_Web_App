...
export class ValidatorService {
  ...
  /** Solo letras (incluye acentos), sin espacios */
  public lettersNoSpaces(input: Nullable<unknown>): boolean {
    const s = this.toStr(input).trim();
    if (!s) return false;
    return /^[\p{L}]+$/u.test(s);
  }
  ...
  public alphanumericNoSpaces(input: Nullable<unknown>): boolean {
    const s = this.toStr(input).trim();
    if (!s) return false;
  ...
  public noSpaces(input: Nullable<unknown>): boolean {
    const s = this.toStr(input);
    return !/\s/.test(s);
  }

  /** CURP estándar de 18 caracteres */
  public curp(input: Nullable<unknown>): boolean {
    const s = this.toStr(input).trim().toUpperCase();
    const re = /^[A-Z]{4}\d{6}[HM][A-Z]{5}[A-Z0-9]{2}$/;
    return re.test(s);
  }

  /** RFC persona física/moral con homoclave */
  public rfc(input: Nullable<unknown>): boolean {
    const s = this.toStr(input).trim().toUpperCase();
